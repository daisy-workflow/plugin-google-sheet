#!/usr/bin/env bash
#
# publish-docker.sh — build & push the Jira plugin image to Docker Hub.
#
# Usage:
...
# ---- config ------------------------------------------------------------------
IMAGE="${IMAGE:-vivek13186/daisy-plugin-google-sheets}"
...
# Confirm Docker Hub login when we're about to push.
if [[ "$PUSH" == "1" ]]; then
